...
- **右侧叠放画廊**：右侧改为 3 张卡片错落叠放效果——前层主卡片（320×210px，微旋转 -1°）、中层（260×175px，旋转 3°）、后层（210×145px，旋转 -4°），每层带递进投影和透明度，增加纵深感。hover 时前层卡片轻微上移放大。
- **左侧装饰底纹**：新增 SVG 装饰元素——两个旋转的细线圆环、半透明圆点阵列、波浪线底纹，营造艺术几何感，不干扰文字阅读。
- **文案胶囊化**：作品标题用绿色胶囊（`rgba(22,199,132,0.1)` 底色），浏览量和 CTA 按钮已移除。
- **横幅高度微调**：从 280px 提升至 320px，为叠放卡片留出空间。
- **响应式**：900px 以下隐藏右侧画廊，装饰底纹降低透明度，仅保留文字区。
- **修改文件**：`src/views/dashboard/Index.vue`（模板结构 + scoped CSS）。

### 首页横幅精简与轮播动画

- **精简文案**：移除浏览量胶囊和"查看热门作品"CTA 按钮，仅保留作品标题胶囊。
- **左侧居中**：文字区增加左侧内边距（`padding-left`），不再紧贴横幅左边缘，视觉更均衡。
- **左侧文字切换动画**：标题胶囊使用 `<Transition name="hero-text">` 包裹，轮播切换时淡入上移 / 淡出上移，`mode="out-in"` 确保过渡平滑。
- **右侧卡片切换动画**：整个叠放卡片组使用 `<Transition name="hero-cards">` 包裹，轮播切换时整体淡入右移 / 淡出左移，`mode="out-in"` 保证流畅。
- **清理无用 import**：移除不再使用的 `View` 图标导入。
- **修改文件**：`src/views/dashboard/Index.vue`（模板 + CSS + script）。

### 首页横幅四卡叠放与点击交互

- **四卡叠放**：从 3 张卡片扩展为 4 张——前层（300×200px，+1°）、back1（240×165px，-3°）、back2（200×140px，+4°）、back3（165×118px，-5°），每层递进缩小、降低透明度，纵深感更强。
- **整体左移**：所有卡片 left 值减小（0/150/260/340px），防止超出横幅右侧边界。
- **点击非首图切换**：点击 back1/back2/back3 卡片时，通过 `switchToHero()` 将该作品切换为前层主卡片，而非跳转详情页。
- **点击首图进详情**：仅点击前层主卡片时触发 `openWork()` 进入作品详情页。
- **数据源扩展**：`featuredWorks` 从 `slice(0, 4)` 改为 `slice(0, 5)`，确保 4 张叠放卡片有足够数据。
- **计数器修正**：轮播计数器显示 `Math.min(featuredWorks.length, 4)`，始终展示 4 张可见卡片的序号。
- **修改文件**：`src/views/dashboard/Index.vue`（模板 + CSS + script）。
